...
### C1. `RespParser::parse_inline` — кривое определение inline-формата
...
### C8. `glob_match` — экспоненциальный backtracking, DoS-вектор
...
### C9. `KvStoreLockFree::del` не retry'ит при contention
...
### C12. `cmd_auth` — пароль в plain text, constant-time не используется
...
| C8 | Баг | ⭐ | Высокий (DoS) | ✅ Срочно |
| C9 | Баг | ⭐⭐ | Высокий (data corrupt) | ✅ Срочно |
| C10 | Tech debt | ⭐ | Низкий | Когда коснёшься |
| C11 | Feature | ⭐⭐⭐ | Средний | После A1 |
| C12 | Security | ⭐ | Высокий | ✅ Срочно |
